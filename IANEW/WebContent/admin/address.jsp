<!----------------------------------------------------------------------------------------------
	...
	Updates:
		...
		08/09/2014 -	Quang Nhan
						changed all ccontact to contact
	...
	<fieldset>
		...
		<!-- to do -->
			...
			<div class="textarea twelve columns">
				...
				<div><s:textarea id="accomodationComments" cssClass="oneLineTextArea" name="contact.accommodationComment" /></div> 
			</div>
		</div>
		...
		<s:if test="%{contact.addressesList.size > 0}">
			<s:hidden id="addressSize" name="contact.addressesList.size" value="%{contact.addressesList.size}"/>
			<s:set name="index" value="contact.addressesList.size" />
			<article id="artAddress" class="hidden">
		</s:if>
		<s:else>
			<s:hidden id="addressSize" name="contact.addressesList.size" value="0"/>
			...
			<section class="sixteen columns curveBorder">
				...
				<s:hidden name="contact.addressesList[%{#index}].id" />
				
				<div class="row">
					<div class="inputfield eight columns">
						<s:label for="street" value="Street: " />
						<div><s:textfield name="contact.addressesList[%{#index}].street" /></div>
					</div>
					<div class="inputfield four columns">
						<s:label for="city" value="Suburb" />
						<div><s:textfield name="contact.addressesList[%{#index}].suburb" /></div>
					</div>
					<div class="inputfield three columns">
						<s:label for="state" value="State:" />
						<div><s:textfield name="contact.addressesList[%{#index}].state" /></div>
					</div>
				</div>
				...
				<div class="row">
					<div class="inputfield four columns">
						...
						<div><s:textfield name="contact.addressesList[%{#index}].country" /></div>
					</div>
					<div class="inputfield four columns">
						<s:label for="postCode" value="Post Code:" />
						<div><s:textfield name="contact.addressesList[%{#index}].postcode" /></div>
					</div>
					<div class="inputfield four columns">
						<s:label for="homePhone" value="Home#:"/>
						<div><s:textfield name="contact.addressesList[%{#index}].homephone" /></div>
					</div>
				</div>
			</section>
		</article>
