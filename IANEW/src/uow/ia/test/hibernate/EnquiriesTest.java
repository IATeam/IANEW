...
public class EnquiriesTest {
	...
	public void f() {
		 ...
		 System.out.println(enquiryList.size());
		 
		 Filter filter = session.enableFilter("enquiryStatusFilter");
		 filter.setParameter("enquiryStatusFilterParam", new Integer(1));
		 
		 List<Enquiries> enquiryList2 = session.createQuery("FROM Enquiries").list();
		 System.out.println(enquiryList2.size());
		 session.disableFilter("enquiryStatusFilter");
